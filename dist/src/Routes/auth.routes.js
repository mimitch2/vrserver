...
router.get('/auth', async (req, res, next) => {
    const callbackUrl = process.env.NODE_ENV === 'production' || process.env.NODE_ENV === 'staging'
        ? `https://${req.hostname}`
...
router.get('/return', async (req, res, next) => {
    ...
    try {
        ...
        if (discogsAccessToken && discogsAccessTokenSecret && userData) {
            const { id: discogsUserId, name, email, uri: discogsUserInfoUri, releases_rated: discogsReleasesRated, } = userData;
            ...
            const appUri = process.env.NODE_ENV === 'development'
