...
const getCollection = async (__, { folder, page, per_page, sort, sort_order }, context) => {
    ...
    const { username, Authorization } = context;
    try {
        const response = await fetch(`${DISCOGS_ENDPOINT}/users/${username}/collection/folders/${folder}/releases${queryParams}`, {
            headers: {
                Authorization,
            },
        });
        const result = await response.json();
        const formatted = (await Promise.all(result?.releases?.map(async (release) => {
...
const getSearch = async (__, { search, type, sort, sort_order, page, per_page, offset, limit }, context) => {
    const params = {
        type,
        sort,
        sort_order,
    ...
    try {
        ...
        if (type === 'release' || type === 'master') {
            formatted = await Promise.all(result?.results?.map(async (release) => {
                const vrRelease = await Release.findOne({ releaseId: release.id });
...
const getWantList = async (__, { page, per_page, sort, sort_order }, context) => {
    ...
    const queryParams = generateQueryParams(isCustomWantListSorted
        ? {
    ...
    try {
        ...
        const result = await response.json();
        const formatted = (await Promise.all(result?.wants?.map(async (release) => {
            const vrRelease = await Release.findOne({ releaseId: release.id });
            return {
                ...release,
                rating: vrRelease?.ratingAvg ?? 0,
...
const getRelease = async (__, { id, instanceId }, context) => {
    ...
    try {
        ...
        if (release) {
            ...
            const { artist, title, ratingsCount, ratingAvg, quietnessAvg, flatnessAvg, clarityAvg, } = release;
            return {
                ...discogsRelease,
                vinylRatingsRelease: {
                    artist,
...
const getMasterReleaseVersions = async (__, { master_id, page, per_page, sort, sort_order, released, country }, context) => {
    const queryParams = generateQueryParams({
        params: {
            format: 'Vinyl',
            page,
    ...
    const { Authorization } = context;
    try {
        const response = await fetch(`${DISCOGS_ENDPOINT}/masters/${master_id}/versions${queryParams}`, {
            headers: {
                Authorization,
            },
        });
        const result = await response.json();
        const formatted = (await Promise.all(result?.versions?.map(async (version) => {
...
const getReleaseInCollection = async (__, { id }, context) => {
    const { username, Authorization } = context;
    try {
        const response = await fetch(`${DISCOGS_ENDPOINT}/users/${username}/collection/releases/${id}`, {
            headers: { Authorization },
        });
        const result = await response.json();
        return {
...
const addToCollection = async (__, { releaseId, folderId }, context) => {
    const { username, Authorization } = context;
    try {
        const response = await fetch(`${DISCOGS_ENDPOINT}/users/${username}/collection/folders/${folderId}/releases/${releaseId}`, {
            method: 'POST',
            headers: { Authorization },
        });
        const result = await response.json();
...
const removeFromCollection = async (__, { folderId, releaseId, instanceId }, context) => {
    const { username, Authorization } = context;
    try {
        const response = await fetch(`${DISCOGS_ENDPOINT}/users/${username}/collection/folders/${folderId}/releases/${releaseId}/instances/${instanceId}
            `, {
            method: 'DELETE',
            headers: { Authorization },
        });
